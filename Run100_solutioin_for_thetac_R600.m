function [ct_m, c_m, d_m] = Run100_solutioin_for_thetac_R600(theta)
...
for t = 1 : avg_times
    ...
    R = 600;
    save ./data/data_main_Initialize_parameters theta R -append
    [o, ~, b] = mainf_RunOPAOc();
    [ct(t), c(t), d(t)] = mainf_cost_CT(o, b);
    
    ts = toc(st);
%     data_ts(t) = ts;
    fprintf('\nRun100_solutioin_for_thetac_R600 %d/%d Time: %f\n', t, avg_times, ts);
end
ct_m = mean(ct);
c_m = mean(c);
d_m = mean(d);
