function [ct_m, c_m, d_m] = Run100_solutioin_for_thetac_fixbf_old(theta)
...
for t = 1 : avg_times
    ...
    save ./data/data_main_Initialize_parameters theta -append
    [o, ~, b] = mainf_RunOPAO_fixbf_oldc();
    [ct(t), c(t), d(t)] = mainf_cost_CT(o, b);
    
    ts = toc(st);
%     data_ts(t) = ts;
    fprintf('\nRun100_solutioin_for_thetac_fixbf_old %d/%d Time: %f\n', t, avg_times, ts);
end
ct_m = mean(ct);
c_m = mean(c);
d_m = mean(d);
