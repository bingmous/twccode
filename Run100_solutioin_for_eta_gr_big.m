function [ct_m, c_m, d_m] = Run100_solutioin_for_eta_gr_big(eta_i_u)
...
for t = 1 : avg_times
    ...
    save ./data/data_main_Initialize_parameters eta_i_u -append
    [ct(t), c(t), d(t)] = mainf_RunOPAO_gr_big();
    ts = toc(st);
%     data_ts(t) = ts;
    fprintf('\nRun100_solutioin_for_eta_gr_big %d/%d Time: %f\n', t, avg_times, ts);
end
ct_m = mean(ct);
c_m = mean(c);
d_m = mean(d);
